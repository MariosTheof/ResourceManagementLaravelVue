<template>
    <div class="container p-4">
        ...
        <form @submit.prevent="deleteResource()">
            ...
            <button class="btn btn-danger modalButton" style="float:right;background: #c51f1a!important;" type="submit" >Submit</button>
        </form>
    </div>
</template>
<script>
export default {
    ...
    methods: {
        deleteResource() {
            this.axios
                .delete('/files/' + this.resource_id)
                .then(response => {
                    if(response.data === 'error') {
                        this.$toastr.e("Something went wrong");
                    } else {
                        this.$modal.hide(this.$parent.name);
                        this.$toastr.s("File was deleted successfully", "Resource Deleted" );
                    }
                })
                .catch(err => {
                    this.$toastr.e("Something went wrong");
